...
procedure ProcessCommandLine;
  ...
  begin
    ...
    if IsppMode then begin
      ...
      WriteStdErr('                                         c: Send output to the compiler after preprocessing. Default state: on.');
      WriteStdErr('                                         e: Keep empty lines and lines with only whitespace. Default state: on.');
      WriteStdErr('                                         v: Verbose mode. Default state: off.');
      WriteStdErr('  -p<letter>(+|-)                      Emulates #pragma parseroption -<letter>(+|-)');
      WriteStdErr('                                         b: Short-circuit boolean evaluation. Default state: on.');
      WriteStdErr('                                         m: Short-circuit multiplication evaluation. Default state: off.');
      WriteStdErr('                                         p: Pascal-style string literals. Default state: on.');
      WriteStdErr('                                         u: Allow undeclared identifiers. Default state: off.');
