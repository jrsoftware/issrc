...
- **No added periods.** Do not add a period to the end of a message that did not have one: Inno Setup appends the period automatically when needed. Keep a trailing period where the English has one. Exception: Thai and Lao remove trailing periods and question marks. If the English ends with `...`, keep the trailing ellipsis, written either as three dots `...` or as a single ellipsis character `…`. Spacing before the ellipsis follows the target language's typography (for example German puts a space before it, while English does not).
