...
- **Fonts** (`DialogFontName`, `DialogFontSize`, `WelcomeFontName`, and so on). `Default.isl` keeps them all commented out. Uncomment only the entries the language genuinely needs to differ; an entry equal to the default stays commented out, so it inherits future default changes. Drop obsolete entries no longer in `Default.isl` (such as `CopyrightFontName`/`CopyrightFontSize`).
...
- **Accelerator keys.** Keep an `&` accelerator wherever the English has one, and do not add one where the English has none. Drop an accelerator only when it cannot be placed on any letter of the translated text; resolve a collision by moving it to another letter rather than dropping it. Within each accelerator scope every accelerator letter must be unique, compared case-insensitively.
- **No added periods.** Do not add a period to the end of a message that did not have one already. Exception: for languages with their own period character, such as Japanese and Chinese, it was added to the end of messages as needed. Keep a trailing period where the English has one. Exception: Thai and Lao remove trailing periods and question marks. If the English ends with `...`, keep the trailing ellipsis, written either as three dots `...` or as a single ellipsis character `…`. Spacing before the ellipsis follows the target language's typography (for example German puts a space before it, while English does not).
